...
public:
	...
	void show_product(string title){
...
void run_xml(string file_name){
	...
	do{
		...
		"0 - Exit the programm\n>>";
		cin >> user_choise;
		if(user_choise == 1) manager_xml->add_product();
		else if(user_choise == 2) manager_xml->add_seller();
		else if(user_choise == 3) manager_xml->add_brend();
		else if(user_choise == 4) manager_xml->add_category();
		else if(user_choise == 5) delete(manager_xml);
		else if(user_choise == 9) {
			cout << "Enter the Product name\n>>";
			cin >> product_name;
			manager_xml->show_product(product_name);
		}
	}while(user_choise != 0);
	system("pause");
}
